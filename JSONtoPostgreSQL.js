...
try {
    ...
    fs.readFile('listings.json', 'utf8', async function (err, data) {
        ...
        var listingData = parsed.entries; // gets array of entries
        ...
        //ADD CODE HERE
        for (var entry of listingData) {
            if (entry.coordinates) {
                const formattedEntry = { // if we needed to create an entry w/ coordinates
                    code: entry.code,
                    name: entry.name,
                ...
                const listing = await Listing.create(formattedEntry);
            }
            else {
                const formattedEntry = { // no coordinates
                    code: entry.code,
                    name: entry.name,
                };
                const listing = await Listing.create(formattedEntry); // add and save listing
            }
        }
    });
} catch (error) {
