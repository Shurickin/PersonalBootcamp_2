...
async function findLibraryWest() {
    ...
    const libWest = await Listing.findOne({ where: { code: 'LBW' } });
    if (libWest === null) {
        console.log('Not found!');
    } else {
        console.log(libWest.name);
    }
}
...
async function updatePhelpsLab() {
    ...
    const phelps = await Listing.findOne({ where: { code: 'PHL' } });
    if (phelps === null) {
        console.log('Not found!');
    } else {
        phelps.address = '1275 Center Dr, Gainesville, FL 32611, United States';
        await phelps.save();
        console.log(phelps.address);
    }
}
